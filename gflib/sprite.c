...
static void DoLoadPokemonSpritePalette(u8 r, u8 g, u8 b, const u16 *src, u16 paletteOffset);
...
u8 LoadTrainerSpritePalette(const struct SpritePalette *palette)
{
    u8 index = IndexOfSpritePaletteTag(palette->tag);
    ...
    else
    {
        ...
        if(FlagGet(FLAG_PASSWORD_TRAINER)){
            DoLoadOpponentSpritePaletteDayNight(palette->data, index * 16);
        }
        else{
            DoLoadSpritePalette(palette->data, index * 16);
        }
...
u8 LoadPokemonSpritePalette(u8 r, u8 g, u8 b, const struct SpritePalette *palette)
{
    u8 index = IndexOfSpritePaletteTag(palette->tag);
    ...
    {
        sSpritePaletteTags[index] = palette->tag;
        DoLoadPokemonSpritePalette(r, g, b, palette->data, index * 16);
        return index;
    }
}
...
u8 LoadPlayerSpritePaletteDayNight(const struct SpritePalette *palette)
{
    u8 index = IndexOfSpritePaletteTag(palette->tag);
    ...
    {
        sSpritePaletteTags[index] = palette->tag;
        DoLoadPlayerSpritePaletteDayNight(palette->data, index * 16);
        return index;
    }
}
...
void DoLoadPokemonSpritePalette(u8 r, u8 g, u8 b, const u16 *src, u16 paletteOffset)
{
    LoadPokemonPalette(r, g, b, src, paletteOffset + 0x100, 32);
}
