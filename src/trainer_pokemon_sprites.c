...
static bool16 DecompressPic(u16 formSpeciesId, u32 personality, bool8 isFrontPic, u8 *dest, bool8 isTrainer)
{
    if (!isTrainer)
    {
        if (isFrontPic)
        {
            LoadSpecialPokePic(&gMonFrontPicTable[formSpeciesId], dest, formSpeciesId, personality, isFrontPic);
        }
        else
        {
            LoadSpecialPokePic(&gMonBackPicTable[formSpeciesId], dest, formSpeciesId, personality, isFrontPic);
        }
    }
    else
    {
        if (isFrontPic)
            DecompressPicFromTable(&gTrainerFrontPicTable[formSpeciesId], dest, formSpeciesId);
        else
            DecompressPicFromTable(&gTrainerBackPicTable[formSpeciesId], dest, formSpeciesId);
    }
    return FALSE;
}
...
static void LoadPicPaletteByTagOrSlot(u16 formSpeciesId, u32 otId, u32 personality, u8 paletteSlot, u16 paletteTag, bool8 isTrainer)
{
    ...
    else
    {
        ...
        {
            sCreatingSpriteTemplate.paletteTag = 0xFFFF;
            LoadPalette(gTrainerFrontPicPaletteTable[formSpeciesId].data, 0x100 + paletteSlot * 0x10, 0x20);
        }
        else
        {
            sCreatingSpriteTemplate.paletteTag = paletteTag;
            LoadSpritePalette(&gTrainerFrontPicPaletteTable[formSpeciesId]);
        }
    }
}
...
u16 CreatePicSprite2(u16 species, u32 otId, u32 personality, u8 flags, s16 x, s16 y, u8 paletteSlot, u16 paletteTag, u8 formId)
{
    ...
    if (DecompressPic(formSpeciesId, personality, flags, framePics, FALSE))
    {
        // debug trap?
        return 0xFFFF;
    }
