...
#define MAX_RELEARNER_MOVES NUM_TECHNICAL_MACHINES//(MAX_LEVEL_UP_MOVES > 50 ? MAX_LEVEL_UP_MOVES : 50) 
...
//TM Move Tutor ---------------------------------------------------
static void CB2_TMMovesMain(void);
static void DoTMTutorMain(void);
static void CreateTMMovesList(void);
static void Task_WaitForFadeOutTM(u8 taskId);
static void CB2_InitTmMoveTutor(void);
static void CB2_InitLearnTmMoveReturnFromSelectMove(void);
//Move Tutor ---------------------------------------------------
...
//TM Move Tutor --------------------------------------------------
void TeachTMTutorMove(void)
{
    ScriptContext2_Enable();
    CreateTask(Task_WaitForFadeOutTM, 10);
    // Fade to black
    BeginNormalPaletteFade(0xFFFFFFFF, 0, 0, 0x10, RGB_BLACK);
}
...
static void Task_WaitForFadeOutTM(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        SetMainCallback2(CB2_InitTmMoveTutor);
        gFieldCallback = FieldCB_ContinueScriptHandleMusic;
        DestroyTask(taskId);
    }
}
...
static void CB2_InitTmMoveTutor(void)
{
    ...
    sMoveRelearnerStruct->moveListMenuTask = ListMenuInit(&gMultiuseListMenuTemplate, sMoveRelearnerMenuSate.listOffset, sMoveRelearnerMenuSate.listRow);
    FillPalette(RGB_BLACK, 0, 2);
    SetMainCallback2(CB2_TMMovesMain);
}
...
static void CreateTMMovesList(void)
{
    ...
    sMoveRelearnerStruct->numMenuChoices = GetTMMovesPokemon(&gPlayerParty[sMoveRelearnerStruct->partyMon], sMoveRelearnerStruct->movesToLearn);
...
static void DoTMTutorMain(void)
{
    ...
    case MENU_STATE_SHOW_MOVE_SUMMARY_SCREEN:
        if (!gPaletteFade.active)
        {
            ShowSelectMovePokemonSummaryScreen(gPlayerParty, sMoveRelearnerStruct->partyMon, gPlayerPartyCount - 1, CB2_InitLearnTmMoveReturnFromSelectMove, GetCurrentSelectedMove());
            FreeMoveRelearnerResources();
        }
        break;
    case 21:
