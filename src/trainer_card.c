...
void TrainerCard_GenerateCardForPlayer(struct TrainerCard *trainerCard)
{
	...
	switch(VarGet(VAR_COSTUME_NUMBER)){
		...
		break;
		case RS_COSTUME:
			SetPlayerCardData(trainerCard, CARD_TYPE_RS);
		break;
		case FRLG_COSTUME:
			SetPlayerCardData(trainerCard, CARD_TYPE_FRLG);
		break;
		case BW_COSTUME:
			SetPlayerCardData(trainerCard, CARD_TYPE_BW);
		break;
	}
...
static void CreateTrainerCardTrainerPic(void)
{
    ...
    else
    {
		...
		switch(VarGet(VAR_COSTUME_NUMBER)){
			...
			break;
			case RS_COSTUME:
				CreateTrainerCardTrainerPicSprite(FacilityClassToPicIndex(sTrainerPicFacilityClass[CARD_TYPE_RS][sData->trainerCard.gender]),
                    TRUE,
                    sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][0],
                    sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][1],
                    8,
			...
			break;
			case FRLG_COSTUME:
				CreateTrainerCardTrainerPicSprite(FacilityClassToPicIndex(sTrainerPicFacilityClass[CARD_TYPE_FRLG][sData->trainerCard.gender]),
                    TRUE,
                    sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][0],
                    sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][1],
                    8,
			...
			break;
			case BW_COSTUME:
                if(sData->trainerCard.gender == GENDER_MALE){
                    CreateTrainerCardTrainerPicSprite(TRAINER_PIC_HILBERT,
                        TRUE,
                        sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][0],
                        sTrainerPicOffset[sData->isHoenn][sData->trainerCard.gender][1],
                        8,
        ...
        SetFrontCharacterTrainerPalette(&gPlttBufferUnfaded[8 * 16], &gPlttBufferUnfaded[8 * 16]);
        SetFrontCharacterTrainerPalette(&gPlttBufferFaded[8 * 16], &gPlttBufferUnfaded[8 * 16]);
    }
}
