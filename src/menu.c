...
void AddTextPrinterForBlackMessage(bool8 allowSkippingDelayWithButtonPress)
{
    void (*callback)(struct TextPrinterTemplate *, u16) = NULL;
    gTextFlags.canABSpeedUpPrint = allowSkippingDelayWithButtonPress;
    AddTextPrinterParameterized2(0, 1, gStringVar4, GetPlayerTextSpeedDelay(), callback, MESSAGEBOX_FONT_COLOR, MESSAGEBOX_BACKGROUND_COLOR, MESSAGEBOX_FONT_SHADOW_COLOR);//check
}
...
void LoadMessageBoxAndBorderGfx(void)
{
    ...
    /*
    //Transparency
    SetGpuReg(REG_OFFSET_BLDCNT, BLDCNT_EFFECT_BLEND | BLDCNT_TGT2_ALL | BLDCNT_TGT1_BG0); //Blend BG0 over the rest
...
void DrawBlackDialogueFrame(u8 windowId, bool8 copyToVram)
{
    CallWindowFunction(windowId, WindowFunc_DrawDialogueFrame);
...
{
    BlitBitmapToWindow(windowId, GetMonIconPtr(speciesId, personality, formId), x, y, 32, 32);
}
